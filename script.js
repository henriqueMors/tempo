const container = document.querySelector('.container');
const search = document.querySelector('.search-box button');
const weatherBox = document.querySelector('.weather-box');
const weatherDetails = document.querySelector('.weather-details');
const error404 = document.querySelector('.not-found');

search.addEventListener('click', () => {
    const APIKey = /*'inserir a chave do api de tempo' API KEY*/;
    const city = document.querySelector('.search-box input').ariaValueMax;

    if (city === '')
        return;

    fetch(/*` INSERIR URL API`*/).then(response => response.json()).then(json => {

        if (json.cod ==='404') {
            container.style.height = '400px';
            weatherBox.style.display = 'none';
            weatherDetails.style.display = 'none';
            error404.style.display = 'block';
            error404.classList.add('fadeIn');
            return;
        }

            error404.style.display = 'none';
            error404.classList.remove('fadeIn');

            const image = doument.querySelector('.weather-box img');
            const temperature = doument.querySelector('.weather-box .temperature');
            const description = doument.querySelector('.weather-box .description');
            const humidity = doument.querySelector('.weather-details .humidity span');
            const wind = doument.querySelector('.weather-details .wind span');

            switch (json.weather[0].main) {
                case 'Clear':
                image.src = 'imgs/clear.png'
                break;

                case 'Rain':
                image.src = 'imgs/rain.png'
                break;

                case 'Snow':
                image.src = 'imgs/snow.png'
                break;

                case 'Clouds':
                image.src = 'imgs/clouds.png'
                break;

                case 'Haze':
                image.src = 'imgs/haze.png'
                break;

                default:
                    image.src = '';

            }

            temperature.innerHTML = `${parseInt(json.main.temp)}<span>ºC</span>`;
            description.innerHTML = `${json.weather[0].description}`;
            humidity.innerHTML = `${json.mais.humidity}%`;
            wind.innerHTML = `${parseInt(json.wind.speed)}Km/h`;

            weatherBox.style.display = '';
            weatherDetails.style.display = '';
            weatherBox.classList.add('fadeIn');
            weatherDetails.classList.add('fadeIn');
            container.style.height = '590px';
    });

});
